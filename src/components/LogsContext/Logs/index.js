...
function Logs({ match, client, classes, history }) {
  ...
  function getLogs() {
    client.query({ query: logsQuery(match.params.id) }).then((data) => {
  ...
  useEffect(() => {
    getLogs();
    client.query({ query: labelsQuery(match.params.id) }).then((data) => {
  ...
  useEffect(() => {
    function filterRows() {
      return rows.filter((w) =>
        moment(w.created).isSameOrAfter(selectedDate, "day")
      );
    }
  ...
  useEffect(() => {
    function filterRows() {
      return rows.filter((w) =>
        w.labels.find((l) => selectedLabels.find((s) => s === l.key))
      );
    }
  ...
  async function deleteLog() {
    await client.mutate({
      mutation: addUpdateLog(match.params.id, {
        ...logToDelete,
        state: "Deleted",
      }),
    ...
    <>
      ...
      <Paper classes={{ root: classes.filtersPaper }}>
        <Grid container justifyContent="flex-end" alignItems="baseline">
          <FormControl
            variant="outlined"
            classes={{ root: classes.filterLabelControl }}
          >
                ...
                <div>
                  {selected.map((value) => (
                    <Chip
                      ...
                      style={{
                        color: "white",
                        backgroundColor: labelsObj[value].color,
                      }}
                      label={labelsObj[value].name}
                    />
                  ))}
                </div>
              )}
      ...
      <List>
        {filteredRows.map((r) => (
          <ListItem
            ...
            button
            component="a"
            href={`/experiments/${match.params.id}/${LOGS_DASH}/${r.key}`}
            classes={{ root: classes.listItem }}
          >
            <ListItemText
              style={{ flex: "none" }}
              primary={r.title}
              secondary={`Opened on ${getDate(r.created)}`}
            />
...
export default compose(withApollo, withRouter, withStyles(styles))(Logs);
