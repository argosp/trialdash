  ...
  async function deleteLog() {
    await client.mutate({
      ...
      update: (cache, mutationResult) => {
        updateCache(cache, mutationResult, logsQuery(match.params.id), 'logs', 'addUpdateLog');
      },
    });
    getLogs();
  }
              ...
              renderValue={(selected) => (
                <div>
                  {selected.map((value) => (
                    <Chip
                      key={value}
                      style={{ color: 'white', backgroundColor: labelsObj[value].color }}
              ...
              {r.labels &&
                r.labels.map((q) => (
                  <Chip
                    component="span"
                    key={q.name}
                    style={{ backgroundColor: q.color, margin: '0 5px', color: 'white' }}
                    label={q.name}
                  />
                ))}
            </div>
