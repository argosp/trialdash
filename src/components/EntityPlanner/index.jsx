...
import {
  changeEntityLocationWithProp,
  findEntitiesChanged,
  getEntityLocationProp,
  getTypeLocationProp,
} from "./EntityUtils";
...
const EntityPlanner = ({
  ...
  const entityWithTrialLocation = (devitem, locationPropOnDevType) => {
    const entityEntityOnTrial = trialEntities.find(
      (ent) => ent.key === devitem.key
    );
    if (entityEntityOnTrial) {
      const locationOnTrial = entityEntityOnTrial.properties.find(
        (entprop) => entprop.key === locationPropOnDevType
      );
      if (locationOnTrial) {
        try {
          ...
          changeEntityLocationWithProp(
            devitem,
            locationPropOnDevType,
            locparsed.coordinates,
            locparsed.name
          );
        } catch (e) {}
      }
  ...
  React.useEffect(() => {
    ...
    const entityTypesAsList = Object.values(entitiesTypes)
      .filter((dtlst) => dtlst.length)
      .flat();
    const newdevs = entityTypesAsList.filter(
      (dt) => dt.name && dt.key && getTypeLocationProp(dt)
    );
    newdevs.sort((a, b) =>
    ...
    newdevs.forEach((devtype) => {
      ...
      client
        .query({
          query: entitiesTrialQuery(experimentId, devtype.key, undefined),
        })
        .then((dataDev) => {
          done += 1;
          setWorking((done / newdevs.length) * 100);
          devtype.items = dataDev.data.entities.map((devitem) =>
            entityWithTrialLocation(devitem, locationProp)
          );
          devtype.items.sort((a, b) =>
            (a.name + ";" + a.key).localeCompare(b.name + ";" + b.key)
          );
  ...
  const handleChangeEntities = (newEntities) => {
    ...
    const changedDetails = changedEntities.map((changed) => {
      ...
      const changeProps = [
        { key: locationProp.key, val: JSON.stringify(locationProp.val) },
      ];
      return {
        key: newDev.key,
        type: "entity",
