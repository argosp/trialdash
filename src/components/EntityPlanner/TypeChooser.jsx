...
export const TypeChooser = ({ selectedType, onChange, showAll, setShowAll, typeOptions }) => (
  <div style={{ width: '100%' }}>
    <div style={{ display: 'inline-block', verticalAlign: 'text-top', margin: 5 }}>
      <InputLabel id="show-all-types" style={{ fontSize: 10 }}>
        Show all
      </InputLabel>
      <Switch
        id="show-all-types"
    ...
    <div style={{ display: 'inline-block', verticalAlign: 'text-top', margin: 5 }}>
      <InputLabel id="select-type" style={{ fontSize: 10 }}>
        Entities Type
      </InputLabel>
      <Select id="select-type" value={selectedType} onChange={(e) => onChange(e.target.value)}>
        {typeOptions.map((opt) => (
