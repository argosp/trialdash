...
export const EntityMap = ({
  ...
  const changeLayerPosition = () => {
    ...
    newPositions[layerChosen] = bounds2arr(
      mapElement.current.leafletElement.getBounds()
    );
  ...
  const showMap =
    layerChosen === "OSMMap"
      ? true
      : (experimentDataMaps || []).find((r) => r.imageName === layerChosen)
          .embedded;

  if (mapElement && mapElement.current && mapElement.current.leafletElement) {
    mapElement.current.leafletElement.options.crs = showMap
      ? CRS.EPSG3857
      : CRS.Simple;
