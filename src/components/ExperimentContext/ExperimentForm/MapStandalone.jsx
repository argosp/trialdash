  ...
  React.useEffect(() => {
    if (
      Math.min(
        ...
        Math.abs(distances.lat),
        Math.abs(distances.lng)
      ) < 1e-6
    ) {
      return;
    }
    ...
    const right =
      Math.round((anchor.lng + (dlng / distances.x) * (imageSize.x - anchor.x)) * 1e9) / 1e9;
    const left =
      Math.round((horizontalPoint.lng - (dlng / distances.x) * horizontalPoint.x) * 1e9) / 1e9;
    const lower = Math.round((anchor.lat - (dlat / distances.y) * anchor.y) * 1e9) / 1e9;
    const upper =
      Math.round(
        (verticalPoint.lat + (dlat / distances.y) * (imageSize.y - verticalPoint.y)) * 1e9
      ) / 1e9;

    if (
      Math.max(
        Math.abs(right - row.right),
        Math.abs(left - row.left),
        Math.abs(lower - row.lower),
        Math.abs(upper - row.upper)
      ) < 1e-6
    ) {
      return;
    }
        ...
        <Grid
          ...
          <Grid item>
            <Grid container spacing={2}>
              <Grid item>
                <NumberTextField
                  value={roundDec(anchor.lng)}
                  onChange={(num) => setAnchor({ ...anchor, lng: num })}
                  label="Anchor X meters"
                  width="150px"
                />
              </Grid>
              <Grid item>
                <NumberTextField
                  value={roundDec(anchor.lat)}
                  onChange={(num) => setAnchor({ ...anchor, lat: num })}
                  label="Anchor Y meters"
                  width="150px"
                />
              </Grid>
            </Grid>
          ...
          <Grid item>
            <Grid container spacing={2}>
              <Grid item>
                <NumberTextField
                  value={roundDec(anchor.x)}
                  onChange={(num) => setAnchor({ ...anchor, x: num })}
                  label="Anchor X pixels"
                  width="150px"
                />
              </Grid>
              <Grid item>
                <NumberTextField
                  value={roundDec(anchor.y)}
                  onChange={(num) => setAnchor({ ...anchor, y: num })}
                  label="Anchor Y pixels"
                  width="150px"
                />
              </Grid>
            </Grid>
          ...
          <Grid item>
            <Grid container spacing={2}>
              <Grid item>
                <NumberTextField
                  value={roundDec(distances.lng)}
                  label="Horizontal Meters"
                  width="150px"
                  onChange={(num) => {
      ...
      <Grid item xs={9}>
        <MapWithImage
          ...
          <MarkedPoint
            ...
            locationToShow={pointLatLngToMeters(anchor)}></MarkedPoint>
          <ChosenMarker key="chosen" center={anchor} />
          <MarkedPoint
            key="horiz"
            ...
            locationToShow={pointLatLngToMeters(horizontalPoint)}></MarkedPoint>
          <MarkedPoint
            key="verti"
            ...
            locationToShow={pointLatLngToMeters(verticalPoint)}></MarkedPoint>
          <DashedPolyline positions={[verticalPoint, anchor, horizontalPoint]}></DashedPolyline>
        </MapWithImage>
      </Grid>
    </Grid>
  );
