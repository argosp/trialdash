...
export const MapStandalone = ({ row, setRow }) => {
  ...
  const distLatLng = (p0, p1) => {
    return Math.sqrt(
      Math.pow(p0.lat - p1.lat, 2) + Math.pow(p0.lng - p1.lng, 2)
    );
  };
      ...
      <Grid item xs={3}>
        ...
        >
          <Grid item>
            ...
            in meters: ({roundDec(row.right - row.left)} x{" "}
            {roundDec(row.upper - row.lower)}) <br />
            dx/dy: ({round9(dx2dy)}) <br />
            dlng/dlat: ({round9(dlng2dlat)}) <br />
            {nonSquarePixels && (
              <>
                <span style={{ color: "red" }}>non-square pixels!</span>
                <IconButton
                  aria-label="expand row"
                  onClick={() => fixNonSquarePixels()}
                >
        ...
        >
          <AnchorPointsDiagonal
            ...
            setAnchorLatLng={({ lat, lng }) =>
              setPointWithoutChange(setAnchor, lat, lng)
            }
            setAnotherLatLng={({ lat, lng }) =>
              setPointWithoutChange(setAnotherPoint, lat, lng)
            }
          ></AnchorPointsDiagonal>
        </MapWithImage>
      </Grid>
