...
const calcBoxFromSpecificCoords = (
  lat0,
  lng0,
  lat1,
  lng1,
...
const MapsEmbedded = ({ row, setRow }) => {
  ...
  const hasNans =
    [row.lower, row.upper, row.left, row.right].findIndex(
      (x) => !Number.isFinite(x)
    ) !== -1;
        ...
        >
          ...
          <Grid item>
            <ControlPointText
              ...
              setPoint={(point) =>
                changeControlPoint(point, selectedControlPoint)
              }
            />
          </Grid>
          <Grid item>
          ...
          <PointsWithLine
            ...
            onPointMove={(p, pointIndex) => {
              if (dragOnMap) {
                ...
                const y =
                  ((p[0] - row.upper) / (row.lower - row.upper)) * imageSize.y;
                const x =
                  ((p[1] - row.left) / (row.right - row.left)) * imageSize.x;
                return changeControlPoint(
                  { lat: p[0], lng: p[1], x: x, y: y },
                  pointIndex
                );
              }
            }}
              ...
              >
                <Grid item>
                  <TextField
                    value={
                      Math.round(
                        distanceMeters(controlPoints[0], controlPoints[1]) * 100
                      ) / 100
                    }
                    onChange={(e) =>
                      changeDistanceMeters(
                        1 - selectedControlPoint,
                ...
                <Grid item>
                  <TextField
                    value={
                      Math.round(
                        distancePixels(controlPoints[0], controlPoints[1]) * 100
                      ) / 100
                    }
                    onChange={(e) =>
                      changeDistancePixels(
                        1 - selectedControlPoint,
                        parseFloat(e.target.value)
                      )
                    }
