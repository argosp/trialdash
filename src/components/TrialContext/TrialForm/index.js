...
constructor(props){
    ...
    this.state = {
        ...
        experimentId:'',
        trialId:'',
        objectiveTypeHeaders:['Fly by', 'Performance Monitoring', 'Flying Camera', 'Blade Inspection'],
        objectiveTypes:[],
...
//builds state to match provided objective parameters
//for easy managing
initiateState = () => {
const objectiveTypes = [];
////////////////////////////////////////////////////
// objectiveStateBuilder.forEach((objectives,index)=> {
//     const parent = objectives[0].parent;
//     const data = {}
//     objectiveTypes.push(parent)
...
graph.sendMutation(trialMutation(trial))
.then(data => { 
    trial.objectives = [];
    trial.id = data.InsertTrial.id
    const last = objectives.length - 1;
    objectives.forEach(async (obj,i) => {
        obj.parent = data.InsertTrial.id
        i === last? obj.isLast = true : obj.isLast = false;
        //graph.sendMutation(objectiveMutation(obj))
        //.then(data => trial.objectives.push(data.InsertObjective))
    })
 })
