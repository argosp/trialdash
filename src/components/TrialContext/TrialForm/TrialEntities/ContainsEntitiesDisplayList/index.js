...
const ContainsEntitiesDisplayList = ({
 ...
 const removeEntityFromArray = (subEntityKey) => {
   const res = findAndUpdateNestedItem([...updatedTrialEntities[entity.key].subItems], subEntityKey , 'subItems');
   setUpdatedTrialEntities((prevTrialEntities) => ({
     ...prevTrialEntities,
 ...
 const findAndUpdateNestedItem = (arr, key, nestingKey) => {
   for (let index = 0; index < arr.length; index++) {
     ...
     if(item.key === key)
       ...
       if(item[nestingKey]) return findAndUpdateNestedItem(item[nestingKey], key, nestingKey);
   }
 }
 const checkHasSubItem = () => {
  return !!updatedTrialEntities[entity.key]  && !!updatedTrialEntities[entity.key].subItems && !updatedTrialEntities[entity.key].subItems.length
 }
 const handleClick = () => {
   if(!openContent && !checkHasSubItem())
     getSubItems();
