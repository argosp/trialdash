...
class TrialForm extends React.Component {
  ...
  initTrial() {
    return {
      ...
      status:
        this.props.trial && this.props.trial.status
          ? this.props.trial.status
          : "design",
      numberOfEntities: this.props.trial
        ? this.props.trial.numberOfEntities
        : 0,
      properties:
        this.props.trial && this.props.trial.properties
          ? [...this.props.trial.properties]
          : [],
      entities:
        this.props.trial && this.props.trial.entities
          ? [
              ...this.props.trial.entities.map((e) => {
  ...
  componentDidMount() {
    ...
    client.query({ query: trialSetsQuery(match.params.id) }).then((data) => {
      ...
      if (!trial) {
        properties = [];
        trialSet.properties.forEach((property) =>
  ...
  onEntityPropertyChange = (entityObj, e, propertyKey) => {
    ...
    let indexOfProperty = trial[entitiesField][
      indexOfEntity
    ].properties.findIndex((property) => property.key === propertyKey);

    if (indexOfProperty === -1) {
      trial[entitiesField][indexOfEntity].properties.push({
        val: value,
        key: propertyKey,
      });
      indexOfProperty =
        trial[entitiesField][indexOfEntity].properties.length - 1;
    } else {
      const property = {
        ...trial[entitiesField][indexOfEntity].properties[indexOfProperty],
      };
      property.val = value;
      trial[entitiesField][indexOfEntity].properties[indexOfProperty] = {
        ...property,
  ...
  updateChangedEntities = (changedEntities, entityObj) => {
    if (changedEntities.findIndex((e) => e.key === entityObj.key) === -1)
      this.setState({
        changedEntities: [...this.state.changedEntities, entityObj],
      });
  };
  ...
  getValue = (key, defaultValue) => {
    ...
    const p =
      properties && properties.length
        ? properties.findIndex((pr) => pr.key === key)
        : -1;
  ...
  removeEntity = (key) => {
    ...
    const entitiesField =
      trial.status === "deploy" ? "deployedEntities" : "entities";
    this.state.trial[entitiesField].splice(
      this.state.trial[entitiesField].findIndex((e) => e.key === key),
      1
    );
  ...
  updateLocation = async (entity) => {
    ...
    updatedTrial[
      !trial.status || trial.status === "design"
        ? "entities"
        : "deployedEntities"
    ] = [entity];
  ...
  setCurrent = (property) => {
    if (property.type === "time")
      this.onPropertyChange(
        { target: { value: moment().format("HH:mm") } },
        property.key
      );
    ...
    return (
      <>
        <div>
                ...
                <StatusBadge
                  ...
                  title={
                    ...
                    >
                      ...
                      {editableStatus && (
                        <PenIcon className={classes.penIcon} />
                      )}
                    </Grid>
                  }
                  color={
                    theme.palette[COLORS_STATUSES[trial.status].color][
                      COLORS_STATUSES[trial.status].level
                    ]
                  }
                />
              )
          ...
          >
            {["design", "deploy", "execution", "complete"].map((i) => (
              <MenuItem
                key={uuid()}
                classes={{ root: classes.menuItem }}
                onClick={(e) =>
                  this.onInputChange({ target: { value: i } }, "status")
                }
              >
          ...
          {trialSet.properties
            ? trialSet.properties.map((property) => (
                <CustomInput
                  ...
                  endAdornment={
                    ["date", "time", "datetime-local"].indexOf(
                      property.type
                    ) !== -1 ? (
        ...
        <ConfirmDialog
          title={
            "There are unsaved changes. Do you want to leave without saving the changes?"
          }
          open={confirmOpen}
