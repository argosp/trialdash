...
async function downloadTrials(client, match, trialSet, displayCloneData) {
  const csvString = await fetchTrialsData(
    client,
    match,
    trialSet,
    displayCloneData
...
async function downloadTrial(args) {
  ...
  const { trial } = data;
  const csvStringTrial = await fetchTrialData(
    trial,
    args.trials,
    args.trialSet,
    args.displayCloneData
  );
  download(csvStringTrial, `trial_${trial.name}`);
  if (trial.fullDetailedEntities && trial.fullDetailedEntities.length) {
    const csvStringEntities = await fetchEntitiesData(
      trial,
      args.trials,
      args.trialSet,
      args.displayCloneData
...
export { downloadTrial, downloadTrials };
