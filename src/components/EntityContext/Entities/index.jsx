...
class Entities extends React.Component {
  ...
  renderTableRow = (entity) => {
    ...
    return (
      <React.Fragment key={entity.key}>
        <StyledTableCell
          className={classnames(classes.tableCell, selectedDT ? classes.selectedRow : '')}
          align="left">
          <Checkbox
            checked={selectedDT}
            ...
            disableRipple
          />
          <span onClick={() => this.activateEditMode(entity)} className={classes.tableCellPointer}>
            {entity.name}
          </span>
        </StyledTableCell>
        {entitiesType &&
        ...
        <StyledTableCell
          className={classnames(classes.tableCellLast, selectedDT ? classes.selectedRow : '')}
          align="right">
          <CustomTooltip
  ...
  generateTableColumns = (entitiesType) => {
    ...
    if (!isEmpty(entitiesType) && !isEmpty(entitiesType.properties)) {
      entitiesType.properties
        .filter((property) => !property.trialField)
        .forEach((property) => {
    ...
    return (
      <LoadingOverlay active={loading} spinner text="Please wait...">
        {this.state.isEditModeEnabled ? (
          ...
          <EntityForm {...this.props} entity={this.state.entity} returnFunc={this.returnFunc} />
        ) : (
          // eslint-disable-next-line react/jsx-wrap-multilines
          <>
            <ContentHeader
              ...
              withBackButton
              backButtonHandler={() =>
                history.push(`/experiments/${match.params.id}/${ENTITIES_TYPES_DASH}`)
              }
              rightDescription={entitiesType.id}
              addButtonHandler={() =>
                history.push(
                  `/experiments/${match.params.id}/${ENTITIES_TYPES_DASH}/${match.params.entitiesTypeKey}/add-entity`
                )
              }
              ...
              addMultipleButtonHandler={() =>
                history.push(
                  `/experiments/${match.params.id}/${ENTITIES_TYPES_DASH}/${match.params.entitiesTypeKey}/add-multiple-entities`
                )
              }
            />
            <ConfirmDialog
...
export default compose(withApollo, withRouter, withStyles(styles))(Entities);
